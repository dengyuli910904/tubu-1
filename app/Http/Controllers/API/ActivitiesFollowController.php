...
class ActivitiesFollowController extends Controller
{
    ...
    public function store(Request $request){
    	$follow = ActivitiesFollow::where('activities_id',$request->input('activities_id'))->where('user_id',$request->input('user_id'))->first();
    	if(!$follow){
    		...
    		return Common::returnErrorResult(400,'您已关注该活动');
    	}
    }
    ...
    public function destory(Request $request){
    	...
    	if($follow){
    		...
    		return Common::returnErrorResult(200,'您还未关注该活动');
    	}
    }
}
